    ...
    translate([cutout?-SLOP:0, cutout?-SLOP:0, cutout?-1:0]) {
        ...
        if(cutout) {
            ...
            translate([0, 0, POWER_TOP_SLIDE/2+1.1]) {
                multmatrix(M) {
                    drawPowerSlideLid(15, extra=cutout?SLOP:0);
                }
            }            
...
module translateForPowerModule() {
    translate([CASE_WIDTH/2 - POWER_WIDTH/2, CASE_HEIGHT - POWER_HEIGHT - 15, -POWER_DEPTH - 3.7]) {
        children();
    }
}
...
module drawScrew(cutout=false, extendHoriz=false, $fn=24) {
    translate([0, 0, -(SCREW_LEN - SCREW_HEAD_DEPTH) - 1.75]) {
        rotate([0, 0, 90]) {
            // Threaded Portion
            cylinder(SCREW_LEN,
...
module drawWiringSpace(cutout=false) {
    if(cutout) {
        ...
        difference() {
            union() {
                cube([DISPLAY_BOARD_WIDTH_INNER + SLOP*4, ARDUINO_LENGTH + 14, POWER_DEPTH + 1]);
        ...
        translate([0, CASE_BORDER + ARDUINO_LENGTH + 20, 0]) {
            cube([5, DISPLAY_BOARD_HEIGHT - 4, POWER_DEPTH - 7.6]);
        ...
        // Area for LED Wiring
        translate([0, ARDUINO_LENGTH + 14 + DISPLAY_BOARD_HEIGHT, 0]) {
            cube([DISPLAY_WIDTH, BEZEL_TOP_TOT - 9, POWER_DEPTH - 7.6]);
            cube([DISPLAY_WIDTH/2 - 4.5, BEZEL_TOP_TOT - 8, POWER_DEPTH - 7.6]);
...
module drawMiddleUpper() {
    ...
    union() {
        difference() {
            translate([0, 0, -1.5]) cube([width, height, 2.49]);
...
module drawWholeUnit(coeff = 0, whichUnit = defaultUnits, whichPart = defaultParts, whichCutout = defaultParts) {
    ...
    // Draw Units
    select(whichUnit) {
        translateAndCutoutForExploded(coeff * 3.5, whichCutout) drawTopHalf();
        translateAndCutoutForExploded(coeff * 2, whichCutout) drawMiddleUpper();
        translateAndCutoutForExploded(coeff * 1, whichCutout) drawMiddleLower();
        translateAndCutoutForExploded(coeff * 0, whichCutout) drawBottom();
    }
    
    select(whichPart) {
        translateForExploded(coeff * 2.5) absoluteDrawDisplayModule(pins=true);
        translateForExploded(coeff * 0.3) absoluteDrawArduino(pins=true);
        translateForExploded(coeff * 0.5) absoluteDrawLED();
        translateForExploded(coeff *-1) absoluteDrawPowerModule();
        translateForExploded(coeff *-1) absoluteDrawScrews();
        translateForExploded(coeff * 3.0) absoluteDrawHeatsets();
    }
...
*difference() {
    drawWholeUnit(0, whichUnit=[0], whichPart=[1]);
    translate([-161, -10, -100])
        cube([200, 200, 200]);
}
...
// Comment this out to show parts
