    ...
    translate([cutout?-SLOP:0, cutout?-SLOP:0, cutout?-1:0]) {
        ...
        if(cutout) {
            ...
            translate([POWER_WIDTH - 5.67, 0, POWER_DEPTH - 5.67 + 1]) {
                rotate([90, 0, 0]) cylinder(h=10, d=5, $fn=4);
            }
...
module drawArduino(cutout=false, pins=false, xtralen_prog = 0, xtralen_side = 0) {
    ...
    if(cutout) {
        translate([-SLOP, -SLOP, -SLOP])
            cube([ARDUINO_WIDTH + SLOP*2,
                  ARDUINO_LENGTH + SLOP*2,
                  ARDUINO_DEPTH + SLOP*2]);
    } else {
        cube([ARDUINO_WIDTH,
              ARDUINO_LENGTH,
              ARDUINO_DEPTH]);
...
module drawScrew(cutout=false, extendHoriz = false, $fn=24) {
    ...
    translate([0, 0, -(SCREW_LEN - SCREW_HEAD_DEPTH) - 1.0]) {
        rotate([0, 0, 90]) {
            // Threaded Portion
            cylinder(SCREW_LEN,
                     d=SCREW_THREAD_OUTER_D + (cutout?SLOP*2:0));
            
            // Head Portion
            translate([0, 0, 0]) {
                cylinder(SCREW_HEAD_DEPTH,
                         d1=SCREW_HEAD_DIAM + (cutout?SLOP*2:0),
                         d2=SCREW_THREAD_OUTER_D + (cutout?SLOP*2:0));
            }
            
            // Screw Channel (for cutout)
            if(cutout) {
                translate([0, 0, -20]) {
                    cylinder(20, d=SCREW_HEAD_DIAM + SLOP*2);
                    if(extendHoriz) {
                        translate([-5, 0, 0]) {
